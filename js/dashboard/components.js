...
export function renderHealthMatrix(matrix = {}) {
    const stages = [
        { key: 'Draft', label: 'Draft', icon: 'edit_note', color: 'text-secondary', bg: 'bg-surface-variant' },
    ...
    return `
    <div class="bg-surface-container-lowest border border-outline-variant/60 rounded-3xl p-5 shadow-sm">
        <div class="flex items-center justify-between mb-4">
            <div class="flex items-center gap-2.5">
                ...
                <div>
                    <h3 class="font-bold text-[16px] text-on-surface">Order Lifecycle Health Matrix</h3>
                    <p class="text-[12px] text-secondary">Real-time state machine pipeline & WIP tracking</p>
                </div>
            </div>
        ...
        <div class="grid grid-cols-3 sm:grid-cols-5 md:grid-cols-9 gap-3">
            ${stages.map(s => {
                ...
                return `
                <div class="flex flex-col items-center p-3 rounded-2xl border border-outline-variant/40 bg-surface-container-low/40 hover:bg-surface-variant/40 transition-all text-center group">
                    ...
                    <div class="text-[11px] font-medium text-secondary truncate w-full mb-0.5">${s.label}</div>
                    <div class="text-[18px] font-extrabold font-mono text-on-surface">${count}</div>
...
export function renderAnomalyWidget(anomalies = []) {
    if (!anomalies || anomalies.length === 0) {
        return `
        <div class="bg-surface-container-lowest border border-outline-variant/60 rounded-3xl p-4 shadow-sm flex items-center gap-3">
            <div class="w-9 h-9 rounded-2xl bg-[#00B386]/10 text-[#00B386] flex items-center justify-center flex-shrink-0">
                <span class="material-symbols-outlined text-[20px]">verified_user</span>
            </div>
            <div>
                <div class="text-[14px] font-bold text-on-surface">No Operational Anomalies</div>
                <div class="text-[12px] text-secondary">All production telemetry metrics are within nominal Z-score baselines (±1.5σ).</div>
            </div>
        </div>`;
    }
    ...
    <div class="bg-surface-container-lowest border border-outline-variant/60 rounded-3xl p-5 shadow-sm">
        <div class="flex items-center gap-2.5 mb-3">
            ...
            <div>
                <h3 class="font-bold text-[16px] text-on-surface">Intelligent Anomaly Detection</h3>
                <p class="text-[12px] text-secondary">Statistically significant metric deviations detected</p>
            </div>
        </div>

        <div class="flex flex-col gap-2.5">
            ${anomalies.map(a => `
            <div class="flex items-start justify-between gap-3 p-3 rounded-2xl bg-error/5 border border-error/20">
                <div class="flex items-start gap-2.5">
                    <span class="material-symbols-outlined text-error text-[20px] mt-0.5">error</span>
                    <div>
                        <div class="text-[13px] font-bold text-on-surface">${a.metric}</div>
                        <div class="text-[12px] text-secondary mt-0.5">${a.message}</div>
                    </div>
                </div>
