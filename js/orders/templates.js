    ...
    const createOrderContent = `
        ...
        <div id="order-step-1" class="wizard-step">
            ...
            <div class="flex flex-col gap-2 relative group mb-4">
                ...
                <div id="customer-search-dropdown" class="hidden absolute top-[100%] left-0 w-full bg-surface border border-outline-variant rounded-xl shadow-lg z-50 mt-1 max-h-60 flex-col overflow-hidden">
...
export function getOrderDetailsHeader(order) {
    ...
    <div class="px-lg pb-md flex justify-between items-start border-b border-outline-variant/30">
        <div>
            <span class="text-[13px] font-bold text-primary mb-1 block">${order.id}</span>
            <h2 class="text-[22px] font-bold text-on-surface leading-tight">${order.customerName || 'Unknown Customer'}</h2>
            <span class="inline-block mt-2 px-3 py-1 rounded-full text-[12px] font-medium ${order.statusColor || 'bg-surface-variant text-secondary'}">${order.status}</span>
        </div>
...
export function getOrderDetailsContent(order) {
    ...
    const incurredCost = order.incurredCost || 0;
    const currentProfit = orderValue - incurredCost;
    const profitPct = orderValue > 0 ? ((currentProfit / orderValue) * 100).toFixed(1) : 0;
    const profitColor = profitPct < 15 ? 'bg-error' : (profitPct < 25 ? 'bg-warning text-warning-container' : 'bg-success');
    ...
    return `
        ...
        <div id="od-tab-overview" class="od-tab-content">
            ...
            <div class="bg-surface-container-lowest rounded-[24px] border border-outline-variant shadow-sm p-lg mb-6">
                ...
                <div class="grid grid-cols-2 gap-4 mb-4">
                    <div class="flex flex-col">
                        ...
                        <span class="text-[18px] font-bold text-on-surface">$${(order.quotedCost || 0).toLocaleString()}</span>
                    </div>
                    <div class="flex flex-col">
                        <span class="text-[12px] font-medium text-secondary mb-1">Actual Incurred Cost</span>
                        <span class="text-[18px] font-bold text-error">$${incurredCost.toLocaleString()}</span>
                    </div>
                </div>
                ...
                <div>
                    <div class="flex justify-between text-[12px] font-medium mb-1">
                        ...
                        <span class="font-bold text-on-surface">$${currentProfit.toLocaleString()} (${profitPct}%)</span>
                    </div>
                    <div class="w-full h-2 bg-surface-container rounded-full overflow-hidden mb-4">
                        <div class="h-full rounded-full transition-all duration-500 ${profitColor}" style="width: ${Math.max(0, profitPct)}%"></div>
                    </div>
            ...
            <h3 class="text-[14px] font-semibold text-secondary uppercase tracking-wider mb-3">Product Specs</h3>
            <div class="bg-surface-container-lowest rounded-2xl p-4 border border-outline-variant shadow-sm mb-6">
                <div class="flex flex-col gap-4">
                    <div>
                        <span class="text-[12px] text-secondary block mb-1">Product</span>
                        <span class="text-[14px] font-medium text-on-surface">${order.product || '—'}</span>
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <span class="text-[12px] text-secondary block mb-1">Total Qty</span>
                            <span class="text-[14px] font-medium text-on-surface">${order.qty ? order.qty.toLocaleString() : '—'}</span>
                        </div>
                        <div>
                            <span class="text-[12px] text-secondary block mb-1">Delivery</span>
                            <span class="text-[14px] font-medium text-on-surface">${order.deliveryDate || '—'}</span>
                        </div>
                        <div>
                            <span class="text-[12px] text-secondary block mb-1">Sizes</span>
                            <span class="text-[14px] font-medium text-on-surface">${order.sizes ? order.sizes.join(', ') : '—'}</span>
                        </div>
                        <div>
                            <span class="text-[12px] text-secondary block mb-1">Colours</span>
                            <span class="text-[14px] font-medium text-on-surface">${order.colours ? order.colours.join(', ') : '—'}</span>
                        </div>
                    </div>
                    <div class="pt-4 border-t border-outline-variant/30">
                        <span class="text-[12px] text-secondary block mb-1">Production Notes</span>
                        <p class="text-[13px] text-on-surface leading-relaxed">${order.notes || 'No notes.'}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div id="od-tab-timeline" class="od-tab-content hidden">
            <!-- Handled by renderTimeline() in index.js -->
            <div id="od-timeline-container" class="pl-2"></div>
        </div>

        <!-- TASKS TAB -->
        <div id="od-tab-tasks" class="od-tab-content hidden">
            <button onclick="window.openTaskSheet()" class="w-full py-3 rounded-xl border border-dashed border-primary text-primary font-bold text-[14px] mb-4 flex items-center justify-center gap-2 active-bg transition-colors">
                <span class="material-symbols-outlined text-[18px]">add_task</span>
                Add Sub-Task
            </button>
            <div id="od-tasks-container" class="flex flex-col gap-3"></div>
        </div>

        <!-- ACTIVITY TAB -->
        <div id="od-tab-activity" class="od-tab-content hidden">
            <!-- Log manual note form -->
            <div class="bg-surface-container-lowest p-3 rounded-2xl border border-outline-variant mb-4">
                <textarea id="od-activity-input" rows="2" placeholder="Log an update or note..." class="w-full bg-transparent border-none focus:ring-0 text-[14px] text-on-surface resize-none p-0 placeholder:text-secondary mb-2"></textarea>
                <div class="flex justify-end border-t border-outline-variant/30 pt-2">
                    <button onclick="window.logActivity()" class="px-4 py-1.5 bg-primary text-white text-[13px] font-bold rounded-lg active-scale transition-apple">Post</button>
                </div>
            </div>
            
            <div id="od-activity-container" class="flex flex-col gap-4"></div>
        </div>
        <div class="h-20"></div>
    `;
}
