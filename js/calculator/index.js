...
window.calcUnified = function() {
    ...
    if (kgInfo) {
        if (totalKg > 0) {
            ...
            setEl('u-kg-val', `${totalKg.toFixed(2)} kg · Yield: ${pcsPerKg.toFixed(2)} pcs/kg`);
        } else {
...
window.openQuotePreview = function() {
    ...
    body.innerHTML = `
        ...
        ${(s.pcsPerKg > 0 || s.weightGms > 0) ? `
        <div class="bg-surface-container-lowest border border-outline-variant/30 rounded-2xl p-4 mb-4 text-[13px]">
            <h4 class="text-[12px] font-bold text-secondary uppercase tracking-wider mb-2 flex items-center justify-between">
                <span>Fabric &amp; Yield Metrics</span>
                <span class="text-primary font-bold">${(s.pcsPerKg || (1000 / s.weightGms)).toFixed(2)} pcs/kg</span>
            </h4>
            <div class="grid grid-cols-2 gap-2 text-[12px]">
                <div class="bg-white/70 dark:bg-slate-800/70 p-2.5 rounded-xl border border-outline-variant/15">
                    <span class="text-secondary block text-[10px] uppercase font-bold">Theoretical Yield</span>
                    <span class="text-on-surface font-black text-[14px]">${(s.pcsPerKg || (1000 / s.weightGms)).toFixed(2)} pcs/kg</span>
                </div>
                <div class="bg-white/70 dark:bg-slate-800/70 p-2.5 rounded-xl border border-outline-variant/15">
                    <span class="text-secondary block text-[10px] uppercase font-bold">Total Fabric Required</span>
                    <span class="text-primary font-black text-[14px]">${(s.pcsPerKg > 0 && s.qty > 0) ? (s.qty / s.pcsPerKg).toFixed(2) + ' kg' : (s.weightGms > 0 && s.qty > 0 ? ((s.weightGms * s.qty) / 1000).toFixed(2) + ' kg' : '—')}</span>
                </div>
            </div>
        </div>` : ''}
    ...
    printWindow.document.write(`
        ...
        <body>
            ...
            <div class="info-grid">
                ...
                <div class="info-section">
                    <h3>Fabric &amp; Yield Parameters</h3>
                    <p><strong>Pcs / Kg (Yield):</strong> ${(s.pcsPerKg || (s.weightGms > 0 ? 1000 / s.weightGms : 0)) > 0 ? (s.pcsPerKg || (1000 / s.weightGms)).toFixed(2) + ' pcs/kg' : '—'}</p>
                    <p><strong>Weight / Piece:</strong> ${s.weightGms > 0 ? s.weightGms.toFixed(1) + ' g' : (s.pcsPerKg > 0 ? (1000 / s.pcsPerKg).toFixed(1) + ' g' : '—')}</p>
                    <p><strong>Total Fabric Required:</strong> ${totalKg > 0 ? totalKg.toFixed(2) + ' kg' : '—'}</p>
                    <p><strong>Fabric Price / kg:</strong> ${s.fabricPriceKg > 0 ? '₹' + s.fabricPriceKg.toFixed(2) + ' (Wastage: ' + (s.wastage || 0) + '%)' : '—'}</p>
                </div>
            </div>
